<template>
  <view class="profile-container">
    <view class="profile-header" v-if="userInfo">
      <view class="avatar">
        <text class="avatar-text">{{ userInfo.username ? userInfo.username.charAt(0).toUpperCase() : userInfo.accountNumber.charAt(0).toUpperCase() }}</text>
      </view>
      <text class="user-name">{{ userInfo.username || userInfo.accountNumber }}</text>
    ...
    <view class="tab-bar">
      <!-- 角色1: 解码、历史、个人 -->
      <block v-if="userInfo && userInfo.role === 1">
        <view
          class="tab-bar-item"
      ...
      <!-- 角色2: 解码、管理、历史、个人 -->
      <block v-else-if="userInfo && userInfo.role === 2">
        <view
          class="tab-bar-item"
          :class="{ 'active': activeTab === 'decode' }"
          @click="switchTab('decode')"
      ...
      <!-- 角色3: 解码、管理、历史、个人 -->
      <block v-else-if="userInfo && userInfo.role === 3">
        <view
          class="tab-bar-item"
          :class="{ 'active': activeTab === 'decode' }"
          @click="switchTab('decode')"
  ...
  methods: {
    async fetchUserInfo() {
      try {
        ...
        const response = await get('/api/User/UserInfo'); // 获取用户信息API端点
    ...
    async updateNickname() {
      ...
      try {
        // 发送修改昵称请求
        const response = await put(`/api/User/UpdateUserName?username=${encodeURIComponent(this.newNickname.trim())}`);
        ...
        if (response.code === 0) {
          ...
          // 更新本地用户信息
          this.userInfo.username = this.newNickname.trim();
    ...
    async updatePassword() {
      ...
      try {
        // 发送修改密码请求
        const response = await patch(`/api/User/UpdatePassword?oldpassword=${encodeURIComponent(this.oldPassword)}&newpassword=${encodeURIComponent(this.newPassword)}&repassword=${encodeURIComponent(this.confirmNewPassword)}`);
        ...
        if (response.code === 0) {
          ...
          // 密码修改成功后，需要调用退出登录API，清空对应的token和个人信息
          try {
            // 发送退出登录请求
            await post('/api/User/Logout');
          } catch (logoutError) {
            console.error('退出登录请求失败:', logoutError); 
          } finally {
            // 清除本地存储的token和用户信息
