      ...
      >
        <view class="card-header" @click="showUserHistory(admin)">
          <view class="user-info">
            ...
            <text class="masked-count">解码次数: {{ admin.maskedCount || 0 }}</text>
          </view>
  ...
  methods: {
    ...
    async fetchAdminList() {
      try {
        ...
        if (this.searchKeyword && this.searchKeyword.trim() !== '') {
          queryParams = `username=${encodeURIComponent(this.searchKeyword.trim())}`;
        }

        const response = await get(`/api/User/FindUserByUsername${queryParams ? '?' + queryParams : ''}`); 
        ...
        if (response.code === 0) {
          ...
          for (let i = 0; i < userList.length; i++) {
            ...
            try {
              // 获取解码次数
              const maskedResponse = await post(`/api/Masked/FindAdminCount?UserId=${user.userId}&Role=${user.role}`);

              if (maskedResponse.code === 0) {
                // 添加解码次数到用户数据
                user.maskedCount = maskedResponse.data || 0;
              } else {
                // 如果获取解码次数失败，默认为0
                user.maskedCount = 0;
              }
            } catch (error) {
              console.error(`获取用户 ${user.userId} 的解码次数失败:`, error);
              // 如果请求出错，默认解码次数为0
              user.maskedCount = 0;
            }
          }
    ...
    async submitUser() {
      ...
      try {
        ...
        const response = await post(`/api/User/AddUser?accountNumber=${encodeURIComponent(this.newUser.account)}&password=${encodeURIComponent(this.newUser.password)}&username=${encodeURIComponent(this.newUser.nickname)}`);
