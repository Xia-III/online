...
namespace Net8WebApi.Controllers
{
        ...
        public class AuthoizeController : ControllerBase
        {
                ...
                /// </summary>
                /// <returns></returns>
                [HttpPost("User/AdminAddUser")]
                public async Task<Result> AdminAddUser(string accountNumber, string username = null)
                {
                        User u = await _userService.FindAsync(u => u.AccountNumber == accountNumber);
                        if (u != null) return Result.Error("账号已被使用");
                        User user = new User();
                ...
                [HttpPost("HXLL/Login")]
                public async Task<Result> HXLLLogin(string username, string password)
                {
                        ////加密后的密码 123456 =>sdlkfjkldsjidaifdaskfaj == sdlkfjkldsjidaifdaskfaj
                        //string pwd = MD5Helper.MD5Encrypt32(password);
                        //数据校验
                        var user = await _userService.FindAsync(c => c.Username == username && c.Password == password && c.Role ==1);
                        if (user != null)
                        {
                                if (user.Status == 0) return Result.Error("账号异常");
                                //登陆成功
